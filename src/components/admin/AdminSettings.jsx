...
const SECTIONS = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'user_account_retention', label: 'Inactive User Account Retention (years)', type: 'number', default: '1' },
    ],
  },
  {
    key: 'notifications',
...
export default function AdminSettings() {
  ...
  const handleSave = () => {
    // In production these would persist to a SystemSettings entity
    setSaved(true);
    ...
    <div className="space-y-6">
      <div className="bg-[#e8ecf1] rounded-3xl shadow-[8px_8px_16px_#c5c9ce,-8px_-8px_16px_#ffffff] p-6">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all ${
                  activeSection === s.key
                    ...
                    : 'text-slate-600 bg-[#e8ecf1] shadow-[3px_3px_6px_#c5c9ce,-3px_-3px_6px_#ffffff]'
                }`}
              >
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <div
                    className={`w-10 h-6 rounded-full relative transition-all ${
                      values[field.key] ? 'bg-[#0ea5e9]' : 'bg-slate-300'
                    }`}
        ...
        <div className="mt-6 flex items-center gap-3">
          <Button
            onClick={handleSave}
            className="bg-gradient-to-r from-[#0ea5e9] to-[#3b82f6] text-white border-0 rounded-xl shadow-[4px_4px_8px_#c5c9ce] gap-2"
          >
